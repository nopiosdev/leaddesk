...
const DailyAttendances =({navigation})=> {
    ...
    const getAttendanceFeed = async (cId) => {
        await GetAttendanceFeed(cId)
            .then(res => {
                var cEmployee=res.result.EmployeeList.filter(x => x.UserId === user.Id);
                console.log('cEmployee'+cEmployee);
                if(cEmployee==null){
                    navigation.navigate('login');
                }

                setemployeeList( res.result.EmployeeList.filter(x => x.UserId != user.Id));
                setdisplayAbleEmployeeList( res.result.EmployeeList);
                setstatusCount(res.result.StatusCount);
                setemployeeDetail(res.result.EmployeeList.filter(x => x.UserId === user.Id)[0])
